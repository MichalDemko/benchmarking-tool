...
func (pe *ParameterEngine) createGeneratorWithConfig(def any, config *Config) (Generator, error) {
	...
	case "choice":
		...
		weights := parseFloatSliceAny(defMap["weights"])
...
func parseFloatSliceAny(raw any) []float64 {
	if raw == nil {
		return nil
	}
	switch w := raw.(type) {
	...
	case []any:
		...
		for _, x := range w {
			out = append(out, getFloatValue(x, 0))
		}
		return out
	default:
		return nil
